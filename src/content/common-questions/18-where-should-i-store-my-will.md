---
title: "Where should I store my will?"
slug: "where-should-i-store-my-will"
category: "Common Questions"
order: 18
region: "Australia"
status: "published"
summary: "Why safe storage and clear access matter once a will needs to be used."
---

A will is only useful if it can be found.

It should be stored somewhere safe, and your executor should know where it is.

Accessibility matters as much as security.

---

## Learn More

**Common Questions**
- [What is a will?](/common-questions/what-is-a-will)
- [Who should I choose as my executor?](/common-questions/who-should-i-choose-as-my-executor)

**Dictionary**
- [Executor](/dictionary/executor)
- [Probate](/dictionary/probate)
- [Grant of Probate](/dictionary/grant-of-probate)

---
